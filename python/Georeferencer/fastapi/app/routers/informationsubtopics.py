...
@router.post("/", response_model=list[schemas.InformationTopicBase])
def create_information_subtopic(information_topic_in: schemas.InformationTopicCreate, db = Depends(get_db)):
    ensure_collection(db, collection_name)
    collection = db.collection(collection_name)
